...
## Delta Method: ejemplo empírico (2)
...
Usemos $g(\hat{\theta}) \sim \mathcal{N}\big(g(\theta), g^{'}(\theta) \cdot SE_{\hat{\theta}} \big)$  !!!
--
 Sólo necesitamos...
...
- $g(\hat{\beta}_{2}) = \frac{1}{N} \sum^{N}_{i=1} \frac{\partial p_{i}}{\partial \text{ym}}= \hat{\beta}_{2} \cdot \Bigg(\frac{1}{1 + e^{-(\beta_{0} + \beta_{1}\text{male}_{i} + \beta_{2}\text{ym}_{i} )}}\Bigg) \Bigg(1 - \frac{1}{1 + e^{-(\beta_{0} + \beta_{1}\text{male}_{i} + \beta_{2}\text{ym}_{i})}}\Bigg)$
...
Dificil, pero no imposible. Largo...
...
1. Asume que la distribución empírica del los datos $\hat{F}(X)$ refleja la distribución de probabilidad de las variables de interés $F(X)$.

--

2. A partir de la muestra obtenén una muestra aleatoria del mismo tamaño que la muestra original (N), con reemplazo:  $X_{b}$

--

3. Calcula el estimate de interés en dicha muestra de la muestra: $\hat{\theta_{b}} = g(X_{b})$.
...
6. Evalúa la distribución del estimate (SE,CI, etc)

---
## Bootstrap Method: ejemplo empírico (1)

A partir de $\ln \frac{p_{i}}{ 1 - p_{i}}    = \beta_{0} + \beta_{1}\text{male}_{i} + \beta_{2}\text{ym}_{i}$, calculemos un IC al 95% para el coeficiente de  "years of marriage": $\beta_{2}$
...
# Iterar función y almacenar resultados 
nreps = 500 
expbetas2_bs <- replicate(nreps,bs_expbeta2() )
