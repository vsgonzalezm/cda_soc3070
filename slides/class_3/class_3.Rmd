...
.content-box-red[
.bold[Pregunta 2]:
¿Cuál es la probabilidad de obtener dos "Caras" con 3 tiros? Es decir, ¿Cuál es la probabilidad de que $Y=2$?
]
...
.content-box-blue[
.bold[Respuesta]: la probabilidad de conseguir dos "Caras" con 3 tiros es:
...
* La probabilidad de obtener una secuencia particular con $k$ "Caras" a partir de $n$ lanzamientos es $p^{k}(1-p)^{n-k}$ 

* Existen ${n \choose k} = \frac{n!}{y! (n-y)!}$ secuencias de este tipo...

--
Por lo tanto,

$$\mathbb{P}(Y=k) = f_{Y}(k) = \frac{n!}{y! (n-y)!} \times p^{k} (1-p)^{n-k}$$
--
...
$$\mathbb{P}(Y=k) = f_{Y}(k) = \frac{n!}{y! (n-y)!}  p^{k} (1-p)^{n-k}$$
La función describe la probabilidad de obtener $k$ éxitos con $n$ intentos.

<br>
--

Cual es la probabilidad de obtener $k$ éxitos con $1$ intento? ( $k \in [0,1]$ ).
--
 Evaluemos la .bold[pf] para $n=1$:
 
\begin{align}
\mathbb{P}(Y=k) = f_{Y}(k) = \begin{cases}
  \frac{1!}{0!1!} p^{0} (1-p)^{1}  = (1-p) &\quad \text{si } k=0 \\ \\
  \frac{1!}{1!0!} p^{1} (1-p)^{0}  = p &\quad \text{si } k=1 
\end{cases}
\end{align}
