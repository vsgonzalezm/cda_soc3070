...
plot + stat_function(fun =var_binom, args = list(n= 1000)) + 
...
plot <- ggplot(data = mydata, mapping = aes(x = x)) +
    ## Entire curve
    geom_path(aes(y=binom,color="red")) +
    geom_path(aes(y=norm,color="blue")) +
...
plot <- data_bin %>% ggplot(aes(x=x)) +
    ...
    geom_line(data =data_norm, aes(y=y, color="blue")) +
    geom_ribbon(data = data_norm %>% filter(x >= qnorm(0.975)), aes(ymin = 0, ymax = y), fill="blue", colour="blue", alpha=0.5) +
    geom_ribbon(data = data_norm %>% filter(x <= qnorm(0.025)), aes(ymin = 0, ymax = y), fill="blue", colour="blue", alpha=0.5) +
    xlim(-4,4) +
...
scale_color_viridis(option = "plasma") + guides(fill=FALSE, color=FALSE) + labs(title="Log-likelihood function",x="p", y="82*log(p) + (100 - 82)*log(1-p)") +
...
$$\hat{\boldsymbol{\beta}}_{MLE} = \text{arg max } \text{ } \mathcal{L}(\boldsymbol{\beta} \mid \boldsymbol{X}) $$
...
- $\mathbb{Var}\big(\hat{p}\big)$ ?

--

  - $\mathbb{Var}\big(\hat{p}\big) = \mathbb{Var}\Big( \frac{\sum x_{i}}{n} \Big) = \mathbb{Var}\Big(\frac{x_{i} + ... + x_{n}}{n}\Big)$
