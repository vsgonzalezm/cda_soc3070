...
<br>
Estructura general del una tabla 2-ways, $I \times J$
...
Para cada frecuencia conjunta $n_{ij}$ en la table existe una probabilidad conjunta asociada $p_{ij}$, tal que

$$p_{ij} = \mathbb{P}(X = i, Y = j)$$


- denota la probabilidad de que una observación muestreada al azar pertenezca en la celda $(i,j)$.

- la colección de probabilidades $p_{ij}$ forma la *distribución conjunta* de $X$ y $Y$ .


Cuando trabajamos con muestras, estas probabilidad se puede estimar a partir de las frecuencias en la tabla:
...
Veamos en caso de nuestro estimador:
...
Al igual que con las frecuencias, podemos obtener la distribución marginal de las variables $X$ y $Y$ a partir su distribución conjunta. 
...
<br>
Estructura general de probabilidades en una  tabla 2-ways, $I \times J$
...
- La distribución condicional $f_{Y,x}(Y \mid X=x)$ es una función que expresa la probabilidad que $Y$ tome cada uno de sus posibles valore $y$'s si $X$ toma valor $x$.

--

- La distribución condicional $f_{Y,X}(Y \mid X)$ es una función que expresa la probabilidad que $Y$ tome cada uno de sus posibles valores $y$'s para $X$ fijo en cada uno de sus valores posible $x$'s.
...
.pull-left[
En nuestro ejemplo podemos construir la distribución condicional  de la variable `everaffair` dado `sex` usando la fórmula genral para probabilidades condicionales:
]
...
En términos más generales, la _distribución condicional_ de la variable de la variable `everaffair`, dado `sex` se estima del siguiente modo:
...
- Las frecuencias en cada celda son generadas por una variable Multinomial $X$ con $I \times 2$ posibles valores $ij$.

--

- Las frecuencias en cada celda son generadas por $I \times 2$ variables Poisson con parámetros $\lambda_{ij}$ respectivamente? 
...
.bold[Pregunta]:
Si la probabilidad de tener un "affair" fuera independiente del género, cual sería la probabilidad de que, al seleccionar una persona al azar, encontremos una mujer que ha tenido un "affair"? 
]
]
...
.bold[Importante]: noten que sólo necesitamos saber la distribución marginal de las variables para calcular las probabilidades y frecuencias esperadas bajo la independencia. 
