...
- $p_{i}/(p_{iJ}) = e^{\beta_{j0}} \cdot e^{\beta_{j1} x_{i1}}  \dots (e^{\beta_{jk}})^{c}$

- $p_{j}/(p_{iJ}) = e^{\beta_{j0}} \cdot e^{\beta_{j1} x_{i^{´}1}}  \dots (e^{\beta_{jk}})^{c+1}$
...
\begin{align}
\frac{p_{i^{´}}/p_{i^{´}J}}{p_{i}/p_{iJ}} &= \frac{e^{\beta_{j0}} \cdot e^{\beta_{j1} x_{i^{´}1}}  \dots (e^{\beta_{jk}})^{c+1}}{e^{\beta_{j0}} \cdot e^{\beta_{j1} x_{i1}}  \dots (e^{\beta_{jk}})^{c}} = e^{\beta_{jk}}
\end{align}
...
<br>
.content-box-yellow[
$$\frac{\partial p_{ij}}{\partial x_{k}} = p_{ij} \cdot \bigg(\beta_{jk} - \sum^{J}_{j=1}p_{ij} \cdot \beta_{jk}\bigg)$$
]
...
.pull-left[
Analizando $$\frac{\partial p_{ij}}{\partial x_{k}} = p_{j} \cdot \bigg(\beta_{jk} - \sum^{J}_{j=1}p_{j} \cdot \beta_{jk}\bigg)$$
]

--

.pull-right[
podemos notar que  $\sum^{J}_{j=1}p_{j} \cdot \beta_{jk} \equiv \overline{\beta_{jk}}, \quad$ el "efecto promedio de" $x_k$. Se desprende que:
]
...
--
  ...
  - $\frac{\partial p_{ij}}{\partial x_{k}} > 0 \quad$  si  $\quad \beta_{jk} > \sum^{J}_{j=1}p_{j} \cdot \beta_{jk}$
...
.pull-left[
si $$\frac{\partial p_{ij}}{\partial x_{k}} = p_{j} \cdot \bigg(\beta_{jk} - \sum^{J}_{j=1}p_{j} \cdot \beta_{jk}\bigg)$$
]
...
.pull-left[
  $p_{ij} =\frac{\beta_{j0} + \beta_{j1}\text{statusquo}_{i} + \beta_{j2}\text{male}_{i}}{1 + \sum^{J-1}_{j=1}  \beta_{j0} + \beta_{j1}\text{statusquo}_{i} + \beta_{j2}\text{male}_{i}}$
...
.pull-right[
$$\frac{\partial p_{ij}}{\partial x_{k}} = p_{j} \cdot \bigg(\beta_{jk} - \sum^{J}_{j=1}p_{j} \cdot \beta_{jk}\bigg)$$
