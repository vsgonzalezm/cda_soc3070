...
.pull-left[
Analizando $$\frac{\partial p_{ij}}{\partial x_{k}} = p_{ij} \cdot \bigg(\beta_{jk} - \sum^{J}_{j=1}p_{ij} \cdot \beta_{jk}\bigg)$$
]

--

.pull-right[
podemos notar que  $\sum^{J}_{j=1}p_{ij} \cdot \beta_{jk} \equiv \overline{\beta_{jk}}, \quad$ el "efecto promedio de" $x_k$. Se desprende que:
]
